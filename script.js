...
class FactCheckAnalyzer extends Analyzer {
    ...
    ratingIndicatesFalse(rating) {
        ...
        return ["false", "fake", "incorrect", "misleading", "debunked", "hoax", "pants on fire"].some(w => r.includes(w));
    }

    ratingIndicatesTrue(rating) {
        const r = rating.toLowerCase();
        return ["true", "correct", "accurate", "mostly true", "confirmed", "verified", "real"].some(w => r.includes(w))
            && !this.ratingIndicatesFalse(rating);
    }

    async analyze(article) {
        // Fallback gracefully if no key is configured
        if (!this.apiKey) {
            return this.noMatchResult("Fact-check API key not configured. Using local rule-based evaluation.");
        }
        ...
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${this.apiKey}`, {
                ...
                body: JSON.stringify({
                    contents: [{
                        parts: [{
                            text: `Fact-check the following news claim against verified real-world news:
...
Return JSON ONLY:
{
  ...
  "explanation": "<1-2 sentence verification summary>"
}`
                        }]
                    }]
                })
            ...
            const credibility = parsed.credibility ?? 50;
            ...
            return {
                ...
                hasMatch: true,
                reason: parsed.explanation || `Fact-check result rated as ${parsed.rating}.`
            };

        } catch (error) {
            console.warn("Fact Check API call failed, using graceful fallback:", error);
            return this.noMatchResult("Fact-check lookup unverified. Evaluated using keyword and language patterns.");
        }
    }
...
class CredibilityReport {
    ...
    calculateOverallScore() {
        ...
        for (const res of this.results) {
            ...
            // Give higher weight to Source Reputation & Fact Check Analyzer
